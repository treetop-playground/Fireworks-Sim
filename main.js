...
const IS_HIGH_END_DEVICE = (() => {
    ...
    // Large screens indicates a full sized display, which is used on desktop or laptops and typically include hyper threading.
    // So a quad core desktop machine has 8 threads. We'll place a higher min threshold there.
    const minCount = window.innerWidth <= 1024 ? 4 : 8;
    return hwConcurrency >= minCount;
})();

// Prevent canvases from getting too large on hi-dpi displays.
...
// NOTE: fullscreen state is synced to store, and the store should be the source
...
const store = {
    ...
    state: {
        ...
        // NOTE: config values used for <select>s must be strings, unless manually converting values to string
        // at render time, and parsing on change.
        config: {
            quality: String(IS_HIGH_END_DEVICE ? QUALITY_HIGH : QUALITY_NORMAL), // will be mirrored to a global variable named `quality` in `configDidUpdate`, for performance.
            shell: 'Random',
            size: IS_DESKTOP
                ? '3' // Desktop default
                : IS_HEADER
...
const helpContent = {
    ...
    shellSize: {
        header: 'Shell Size',
        body: 'The size of the firework. Modeled after real firework shell sizes, larger shells have bigger bursts with more stars, and sometimes more complex effects. However, larger shells also require more processing power and may cause lag.'
    },
    quality: {
        header: 'Quality',
        boyd: 'Overall graphics quality. If the animation is not running smoothly, try lowering the quality. High quality greatly increases the amount of sparks rendered and may cause lag.'
    },
    skyLighting: {
        header: 'Sky Lighting',
...
// Converts appNodes selectors to DOM nodes
...
// Invisible stars need an identifier, even though they won't be rendered - physics still apply.
const COLOR_CODES_W_INVIS = [...COLOR_CODES, INVISIBLE];
// Mpa of color codes to their index in the array. Useful for quickly determining if a color has already been update in a loop.
const COLOR_CODE_INDEXES = COLOR_CODES_W_INVIS.reduce((obj, code, i) => {
    obj[code] = i;
    return obj;
}, {});
// Tuples is a map of keys by colors codes (hex) with values of {r, g, b} tuples (still just objects).
const COLOR_TUPLES = {};
...
// Get a random shell, not including processing intensive variants
// NOTE: this is only random when "Random" shell is selected in config.
// Also, this does not create the shell, only return the factory function.
        ...
        for (let i = stars.length - 1; i >= 0; i = i - 1) {
            ...
            if (star.life <= 0) {
                ...
                if (star.life < star.transitionTime) {
                    ...
                    if (star.strobe) {
                        // Strobes int he following pattern: on:off:off:on:off:off in increments of `strobeFreq` ms.
...
function render(speed) {
    ...
    // These must also be drawn using source-over due to Safari. Seems rendering the gradients using lighten draw large block boxes instead.
    // Thankfully, these burst flasges looks pretty much the same either way.
...
// Values are flexible - `start` and `arcLength` can be negative, and `randomness` is simply a multiplier for random additon.
function createParticleArc(start, arcLength, count, randomness, particleFactory) {
    ...
    if (end > start) {
        ...
        // V8 deoptimizes with let compound assignment
...
function createBurst(count, particleFactory, startAngle = 0, arcLength = PI_2) {
    ...
    for (let i = 0; i <= C_HALF; i++) {
        ...
        // Each particles needs a bit of randomness to improve appearance.
    ...
    burst(x, y) {
        ...
        if (typeof this.color === 'string') {
            if (this.color === 'random') {
                color = null; // false value creates random color in starFactory
            } else {
                color = this.color;
            }

            // Rings have positinal randomness, but are rotated randomly
            if (this.ring) {
            ...
            throw new Error('Invalid shell color. Expected string or array of strings, but received: ' + this.color);
        }
